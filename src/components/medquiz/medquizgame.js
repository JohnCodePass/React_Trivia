...
import Medquiz from './medquizQA'
import { Link } from 'react-router-dom'
import ReactPlayer from 'react-player'

import '../quizgame/quizGame.css';

export default class Mquiz extends Component {
    constructor(props) {
        super(props)
        this.state = {
            userAnswer: null,
    ...
    loadGame = () => {
        ...
        this.setState(() => {
            return {
                question: Medquiz[currentIndex].question,
                options: Medquiz[currentIndex].options,
                answer: Medquiz[currentIndex].answer
            }
        })
    }
    ...
    finishCheck = () => {
        ...
        if (currentIndex === Medquiz.length - 1) {
            if (userAnswer === answer) {
                this.setState({
                    score: score + 1
                })
            ...
            <>
                {quizEnd === false ? (
                    <Container className='gameArea d-flex justify-content-center flex-column'>
                        ...
                        <Row>
                            <Col>
                                <h3>{`Question ${currentIndex + 1} of ${Medquiz.length}`}</h3>
                            </Col>
                            <Col>
                                <h2>Score is {score} / {Medquiz.length}</h2>
                            </Col>
                            <Col>
                                <h1>{timer} secs</h1>
                                <>
                                    {timer === 0 && currentIndex === Medquiz.length - 1 ? (
                                        this.finishCheck()
                                    ) :
                                        timer === 0 && currentIndex !== Medquiz.length - 1 ? (
                                            this.nextQuestion()
