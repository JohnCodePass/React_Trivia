...
import Easyquiz from './easyquizQA'
...
export default class Quiz extends Component {
    ...
    loadGame = () => {
        ...
        this.setState(() => {
            return {
                question: Easyquiz[currentIndex].question,
                options: Easyquiz[currentIndex].options,
                answer: Easyquiz[currentIndex].answer
            }
        })
    }
    ...
    finishCheck = () => {
        ...
        if (currentIndex === Easyquiz.length - 1) {
            if (userAnswer === answer) {
                this.setState({
                    score: score + 1
                })
            ...
            <>
                {quizEnd === false ? (
                    <Container className='gameArea d-flex justify-content-center flex-column'>
                        ...
                        <Row>
                            <Col>
                                <h3>{`Question ${currentIndex + 1} of ${Easyquiz.length}`}</h3>
                            </Col>
                            <Col>
                                <h2>Score is {score} / {Easyquiz.length}</h2>
                            </Col>
                            <Col>
                                <h1>{timer} secs</h1>
                                <>
                                    {timer === 0 && currentIndex === Easyquiz.length - 1 ? (
                                        this.finishCheck()
                                    ) :
                                        timer === 0 && currentIndex !== Easyquiz.length - 1 ? (
                                            this.nextQuestion()
                        ...
                        { currentIndex === (Easyquiz.length - 1) ? (
                            <Row className='d-flex justify-content-center'>
                                <Col xl={4} md={8}>
